...
resource "google_service_account" "sa_github_cicd" {
  ...
  account_id   = "sa-gha-${var.cicd_project_id}"
...
#################################################
######### CD IAC WIF Account UAT ENV ############
#################################################

resource "google_project_iam_member" "github_actions_access_cicd_uat" {
...
resource "google_service_account_iam_binding" "github_account_binding_cicd" {
  ...
  members = [
    ...
    "principalSet://iam.googleapis.com/${google_iam_workload_identity_pool.github_pool_cicd.name}/attribute.repository/groupm-global/mcm-createlift-infra",
  ]
}
